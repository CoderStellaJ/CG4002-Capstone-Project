    ...
    def handleNotification(self, cHandle, data):
        ...
        for idx in range(len(beetle_addresses)):
            if global_delegate_obj[idx] == self:
                ...
                beetle1_handshake_flag_dict.update( # remove this if fixed handshaking stage bug
                ...
                if beetle_addresses[idx] in beetle1_buffer_dict:
                    for char in beetle1_buffer_dict[beetle_addresses[idx]]:
                        if char == 'A' or beetle1_handshake_flag_dict[beetle_addresses[idx]] is True:
                            if char == 'A':
                                continue
                            if beetle1_handshake_flag_dict[beetle_addresses[idx]] == True:
                            ...
                            if char != 'D' and char != '.' and char != ',' and (float_flag_1 is True or timestamp_flag_1 is True): # integers belonging to floating point values
                                ...
                                if comma_count_1 == 2: # already past timestamp value
                                    timestamp_flag_1 = False
                                    beetle1_data_dict[len]
                                elif comma_count_1 > 2:
...
def getBeetleData(beetle_peri):
    while True:
        try:
            print("before waitForNotifications")
            if beetle_peri.waitForNotifications(20):
                ...
                """
                continue
            print("after waitForNotifications")
