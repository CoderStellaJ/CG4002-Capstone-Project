...
void loop() {
  while (1) {
    if (is_repeat_dance) {
      while (1) {
        ...
        if (abs(yawDiff) >= 5 || abs(pitchDiff) >= 5 || abs(rollDiff) >= 5) {
          // Get the initial timestamp of this new dance move
          timestamp = millis();
          // Loop to get 128 samples from MPU6050 at the frequency of 20Hz
          for (int i = 0; i < 30; i++) {
  ...
  while (1) {
    ...
    while (1) {
      if (Serial.available() && Serial.read() == 'R') { // repeat position movement
        is_repeat_position = true;
        break;
      } else if (Serial.available() && Serial.read() == 'P') { // position movement success
        is_repeat_position = false;
        break;
      }
    }
    if (is_repeat_position) {
