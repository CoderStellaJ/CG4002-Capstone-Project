...
def predict(model, prev_accuracy):
    ...
    for x in range(len(test_data_array)):
        data = np.append(np.asarray(test_data_array[x, 0:30], dtype = np.float32), [0, 0])
    ...
    if (accuracy > prev_accuracy):
        bo.export_finn_onnx(model, (1, 32), './qnn_harnn_model_' + str(accuracy) +'.onnx')
...
for epoch in range(epochs):
  ...
  for x in range(len(train_data_array)):
    data = np.append(np.asarray(train_data_array[x, 0:30], dtype = np.float32), [0, 0])
    data = data.astype(np.float32)
    data = torch.tensor([data])
    data.requires_grad = True;
