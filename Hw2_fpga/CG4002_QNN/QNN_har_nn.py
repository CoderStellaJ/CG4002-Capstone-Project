...
class QNN_HARnn(torch.nn.Module):
  def __init__(self):
    ...
    self.linear1 = qnn.QuantLinear(8, 512, bias=False, weight_quant_type=QuantType.INT, weight_bit_width=8, weight_scaling_stats_op = StatsOp.MAX, weight_scaling_stats_sigma=0.001, weight_scaling_per_output_channel = True, weight_narrow_range = True, weight_bit_width_impl_type=BitWidthImplType.CONST)
    ...
    self.linear2 = qnn.QuantLinear(512, 3, bias=False, weight_quant_type=QuantType.INT, weight_bit_width=8, weight_scaling_stats_op = StatsOp.MAX, weight_scaling_stats_sigma=0.001, weight_scaling_per_output_channel = False, weight_narrow_range = True, weight_bit_width_impl_type=BitWidthImplType.CONST)
    ...
    #self.linear3 = qnn.QuantLinear(64, 3, bias=False, weight_quant_type=QuantType.BINARY, weight_bit_width=1, weight_scaling_stats_op = StatsOp.AVE, weight_scaling_stats_sigma=0.001, weight_scaling_per_output_channel = False, weight_narrow_range = True, weight_bit_width_impl_type=BitWidthImplType.CONST)
...
labels = {
  "shoutout": torch.tensor([0]),
  "weightlifting": torch.tensor([1]),
  "muscle": torch.tensor([2]),
}
...
def predict(model, prev_accuracy):
    ...
    for x in range(len(test_data_array)):
        data = np.append(np.asarray(test_data_array[x, 0:6], dtype = np.float32), [0, 0])
    ...
    if (accuracy > prev_accuracy):
        bo.export_finn_onnx(model, (1, 8), './qnn_harnn_model_' + str(accuracy) +'.onnx')
...
for epoch in range(epochs):
  ...
  for x in range(len(train_data_array)):
    data = np.append(np.asarray(train_data_array[x, 0:6], dtype = np.float32), [0, 0])
    data = data.astype(np.float32)
    data = torch.tensor([data])
    data.requires_grad = True;
